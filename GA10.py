...
class GeneticAlgorithm:
            ...
            for d in range(0, self.single_population_update):
                ...
                if d == self.single_population_update - 1:
                    ...
                    for s in range(0, len(self.defenders[0].get_strategies())):
                        ...
                        self.def_strategy_population_average_average[s].append(np.mean(self.def_strategy_population_average[s]))

                        new_strategies.append(Periodic(self.def_strategy_population_average_average[s][-1]))
    ...
    def read_from_file(self, number):
        ...
        file = Path(self.ga_properties.get('file_location') + 'attacker_rates')
        if file.exists():
            with open(file, 'r') as f:
                s = f.read()
